...
func TestReader_Listen(t *testing.T) {
	...
	testOffsetStr := "test-stream//0"
	testMessage := &natslib.Message{
		Subject: "test-stream",
	...
	tests := []struct {
		name          string
		reader        func(doneChan chan struct{}) *natsmocks.Reader
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := &Reader{
				...
				offsetParser: &natsmocks.OffsetParser{
					ToStringFn: func(o *natslib.Offset) string { return testOffsetStr },
				},
			}
