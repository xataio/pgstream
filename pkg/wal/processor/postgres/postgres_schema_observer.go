...
func (o *pgSchemaObserver) queryGeneratedColumnNames(ctx context.Context, schemaName, tableName string) (map[string]struct{}, error) {
	...
	// Filter out columns that are truly auto-generated and cannot be overridden:
	// - attgenerated != '': computed/stored generated columns
	// - attidentity = 'a': GENERATED ALWAYS AS IDENTITY (cannot be overridden)
	// We intentionally exclude attidentity = 'd' (GENERATED BY DEFAULT AS IDENTITY)
	// because those columns CAN accept explicit values during COPY, and we MUST
	// preserve the original primary key values from the source.
