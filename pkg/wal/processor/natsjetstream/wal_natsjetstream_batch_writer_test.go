...
func TestBatchNATSJetstreamWriter_ProcessWALEvent(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		batchSender       *batchmocks.BatchSender[natslib.Message]
		walDataToDDLEvent func(*wal.Data) (*wal.DDLEvent, error)

		wantMsgs []*batch.WALMessage[natslib.Message]
		wantErr  error
	}{
		{
			name:        "ok",
			walEvent:    testWalEvent,
			batchSender: batchmocks.NewBatchSender[natslib.Message](),

			wantMsgs: []*batch.WALMessage[natslib.Message]{
				batch.NewWALMessage(natslib.Message{
					Key:   []byte(testSchema),
					Value: testBytes,
				}, testCommitPosition),
			},
		...
		},
		{
			...
			wantMsgs: []*batch.WALMessage[natslib.Message]{
				batch.NewWALMessage(natslib.Message{}, testCommitPosition),
			},
			wantErr: nil,
		},
		{
...
func TestBatchNATSJetstreamWriter_sendBatch(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name       string
		writer     *natsmocks.Writer
		checkpoint checkpointer.Checkpoint
		batch      *batch.Batch[natslib.Message]
	...
	}{
		{
			name: "ok",
			writer: &natsmocks.Writer{
				WriteMessagesFn: func(ctx context.Context, i uint64, msgs ...natslib.Message) error {
		...
		},
		{
			...
			batch: batch.NewBatch([]natslib.Message{}, nil),
