...
func (c *DDLColumn) IsGenerated() bool {
	// Only treat GENERATED ALWAYS AS IDENTITY as "generated" (cannot be overridden).
	// GENERATED BY DEFAULT AS IDENTITY (Identity = "BY DEFAULT") accepts explicit
	// values and MUST be included in COPY to preserve original primary key values.
	return c.Generated || (c.Identity != nil && *c.Identity == "ALWAYS")
}
