...
func TestNewTimestampTransformer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "error - min_timestamp larger tah max_timestamp",
			generator: transformers.Deterministic,
			params: transformers.Parameters{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			transformer, err := NewTimestampTransformer(tt.generator, tt.params)
			if tt.wantErr != nil {
...
func TestTimestampTransformer_Transform(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			transformer, err := NewTimestampTransformer(tt.generatorType, tt.params)
			require.NoError(t, err)
			require.NotNil(t, transformer)
			...
			minTimestamp, err := time.Parse(time.RFC3339, tt.params["min_timestamp"].(string))
			require.NoError(t, err)
			maxTimestamp, err := time.Parse(time.RFC3339, tt.params["max_timestamp"].(string))
			require.NoError(t, err)

			truncatePart, _, err := transformers.FindParameter[string](tt.params, "truncate_part")
			require.NoError(t, err)
			...
			require.GreaterOrEqual(t, result.Unix(), minTimestamp.Unix())
			require.LessOrEqual(t, result.Unix(), maxTimestamp.Unix())
		})
	}
}
