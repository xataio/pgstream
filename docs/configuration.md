...
| PGSTREAM_POSTGRES_REPLICATION_RECEIVE_TIMEOUT           | 90s                          | No       | How long a single read on the replication connection waits. Postgres sends a keepalive every `wal_sender_timeout`/2, so silence for longer means the stream is gone and the connection is reset. A negative value removes the bound, for a source with `wal_sender_timeout = 0`.                              |
