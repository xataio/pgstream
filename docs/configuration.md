    ...
    snapshot: # when mode is snapshot or snapshot_and_replication
      ...
      schema: # when mode is full or schema
        pgdump_pgrestore:
          ...
          index_constraint_session_settings: # optional PostgreSQL name=value session settings applied only while restoring indexes and constraints. Unset or empty preserves existing behavior
...
| PGSTREAM_POSTGRES_SNAPSHOT_INDEX_CONSTRAINT_SESSION_SETTINGS | []                       | No       | Space-separated PostgreSQL `name=value` session settings applied only while restoring indexes and constraints, for example `maintenance_work_mem=4GB max_parallel_maintenance_workers=4`. Unset or empty preserves existing behavior.                                           |
