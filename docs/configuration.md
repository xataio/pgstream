...
| PGSTREAM_POSTGRES_SNAPSHOT_ROLES_SNAPSHOT_MODE       | "no_passwords"               | No       | When using `pg_dump`/`pg_restore` to snapshot schema for Postgres targets, option to enable/disable snapshotting roles. Can be set to "no_passwords" to snapshot roles, without passwords.                                                                                                                   |
