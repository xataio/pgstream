-- Fix CREATE TRIGGER events being silently dropped
--
-- pg_event_trigger_ddl_commands() returns a row for CREATE TRIGGER commands,
-- but leaves schema_name NULL, since triggers are not schema-qualified
-- catalog objects the way tables or functions are. emit_ddl() picks the
-- schema from the first row returned by that function and returns early
-- when it is NULL, so CREATE TRIGGER events were never emitted. DROP
-- TRIGGER events go through the sql_drop event and
-- pg_event_trigger_dropped_objects(), which does report the schema, so
-- they were emitted. In strict mode, replicating the DROP without the
-- matching CREATE causes the target to fail.
--
-- Resolve the schema for trigger objects from the table they are defined on.
CREATE OR REPLACE FUNCTION pgstream.get_trigger_schema(trigger_oid oid)
RETURNS text
LANGUAGE SQL
STABLE
...
BEGIN
    ...
    ELSIF tg_event = 'ddl_command_end' THEN
        ...
        -- CREATE TRIGGER reports a NULL schema_name, so resolve it via the
        -- trigger's table.
        SELECT COALESCE(
            schema_name,
            CASE WHEN object_type = 'trigger' THEN pgstream.get_trigger_schema(objid) END
        ) INTO rec_schema_name
