-- Add sequences to log schema function
...
BEGIN
    ...
    IF tg_event = 'sql_drop' THEN
        IF tg_tag = 'DROP SCHEMA' THEN
            ...
            END IF;
        ELSIF tg_tag IN ('DROP TABLE', 'DROP INDEX', 'DROP MATERIALIZED VIEW', 'DROP SEQUENCE') THEN
            SELECT objid, schema_name
            INTO rec_objid, rec_schema_name
            FROM pg_event_trigger_dropped_objects()
            WHERE object_type IN ('table', 'index', 'materialized view', 'sequence')
            LIMIT 1;
    ...
    ELSIF tg_event = 'ddl_command_end' THEN
        IF tg_tag = 'CREATE SCHEMA' THEN
            ...
            LIMIT 1;
        ELSIF tg_tag IN ('CREATE TABLE', 'ALTER TABLE', 'CREATE INDEX', 'CREATE UNIQUE INDEX', 'ALTER INDEX', 'SELECT INTO', 'CREATE TABLE AS', 'CREATE MATERIALIZED VIEW', 'ALTER MATERIALIZED VIEW', 'CREATE SEQUENCE', 'ALTER SEQUENCE') THEN
            SELECT schema_name INTO rec_schema_name
            FROM pg_event_trigger_ddl_commands()
            WHERE object_type IN ('table', 'table column', 'index', 'materialized view', 'sequence')
