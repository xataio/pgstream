-- Ensure SELECT INTO statements are tracked like any other CREATE TABLE operations
-- and drop the redundant index trigger (CREATE/ALTER events are already captured
-- by pgstream_log_schema_create_alter_table).
...
BEGIN
    ...
    ELSIF tg_event = 'ddl_command_end' THEN
        IF tg_tag = 'CREATE SCHEMA' THEN
            ...
            LIMIT 1;
        ELSIF tg_tag IN ('CREATE TABLE', 'ALTER TABLE', 'CREATE INDEX', 'ALTER INDEX', 'SELECT INTO', 'CREATE TABLE AS') THEN
