-- Migration: Add full index replication support (triggers, log_schema, get_schema)

-- 1. Add event triggers for index operations
...
CREATE EVENT TRIGGER pgstream_log_schema_drop_index
  ON sql_drop
  WHEN tag IN ('DROP INDEX')
  EXECUTE FUNCTION pgstream.log_schema();

-- 2. Extend pgstream.log_schema() to handle index operations
...
BEGIN
    ...
    ELSIF tg_event = 'ddl_command_end' THEN
        IF tg_tag IN ('CREATE SCHEMA', 'CREATE TABLE', 'ALTER TABLE', 'CREATE INDEX', 'ALTER INDEX') THEN
            SELECT schema_name INTO rec_schema_name FROM pg_event_trigger_ddl_commands()
            WHERE object_type IN ('schema', 'table', 'table column', 'index')
...
-- 3. Extend pgstream.get_schema() to include index definitions
