...
--
-- See migrations/postgres/core/4_fix_create_trigger_schema.up.sql for the
-- full explanation: pg_event_trigger_ddl_commands() leaves schema_name NULL
-- for CREATE TRIGGER, so emit_ddl() never emitted those events, while the
-- matching DROP TRIGGER was. Resolve the schema for trigger objects using
-- pgstream.get_trigger_schema(), defined by the core migration above.
